...
- Name: Mermaid Diagram Rendering >> should render complex Mermaid diagram correctly
- Location: /mnt/network_repo/DiagramAI-1/DiagramAI/tests/e2e/mermaid-rendering.spec.ts:68:7
...
Error: locator.textContent: Error: strict mode violation: locator('h1') resolved to 2 elements:
    ...
    2) <h1 class="text-2xl font-bold text-gray-900">AI Agent Architecture Demo</h1> aka getByRole('heading', { name: 'AI Agent Architecture Demo' })

Call log:
  - waiting for locator('h1')

    at /mnt/network_repo/DiagramAI-1/DiagramAI/tests/e2e/mermaid-rendering.spec.ts:79:44
```
...
- main:
  - button "← Back"
  - heading "AI Agent Architecture Demo" [level=1]
  - paragraph: Demo diagram created to simulate MCP agent functionality
  - text: 📝 Mermaid 🌐 Public
  - link "Edit Diagram":
    - /url: /editor?id=74a7ac31-15cc-4c87-a522-2ce85435d963
  - paragraph: ❌ Rendering Error
  - paragraph: Failed to render diagram
  - group: View Mermaid Syntax
  - heading "Diagram Information" [level=3]
  - text: Created:May 30, 2025 at 07:53 AM Last Modified:May 30, 2025 at 07:53 AM Format:mermaid Visibility:Public
- contentinfo:
...
>  79 |     const title = await page.locator('h1').textContent()
  ...
  128 |     
  129 |     // Take a screenshot for visual verification
  130 |     await page.screenshot({ 
  131 |       path: 'DiagramAI/tests/screenshots/complex-mermaid-test.png',
  132 |       fullPage: true 
  133 |     })
  134 |     console.log('Screenshot saved to tests/screenshots/complex-mermaid-test.png')
  135 |   })
  136 |
  137 |   test('should display console logs and errors', async ({ page }) => {
  138 |     console.log('Capturing browser console logs...')
  139 |     
  140 |     const logs: string[] = []
  141 |     const errors: string[] = []
  142 |     
  143 |     // Capture console logs
  144 |     page.on('console', msg => {
  145 |       const logMessage = `[${msg.type()}] ${msg.text()}`
  146 |       logs.push(logMessage)
  147 |       console.log('Browser console:', logMessage)
  148 |     })
  149 |     
  150 |     // Capture page errors
  151 |     page.on('pageerror', error => {
  152 |       const errorMessage = `Page error: ${error.message}`
  153 |       errors.push(errorMessage)
  154 |       console.log('Browser error:', errorMessage)
  155 |     })
  156 |     
  157 |     // Navigate to simple diagram and capture logs
  158 |     await page.goto('http://localhost:3000/diagram/f50ff3c6-f439-4952-9370-49d686372c22')
  159 |     await page.waitForLoadState('networkidle')
  160 |     await page.waitForTimeout(3000)
  161 |     
  162 |     console.log('\n=== BROWSER CONSOLE LOGS ===')
  163 |     logs.forEach(log => console.log(log))
  164 |     
  165 |     console.log('\n=== BROWSER ERRORS ===')
  166 |     if (errors.length > 0) {
  167 |       errors.forEach(error => console.log(error))
  168 |     } else {
  169 |       console.log('No browser errors detected')
  170 |     }
  171 |     
  172 |     // The test should not fail due to console logs, but we want to see them
